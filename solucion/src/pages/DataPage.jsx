...
const getDateLabel = (day) => `${weekDays[(day + 1) % 7]} ${day} Sep`
  ...
  return (
    <div className="site-shell booking-page">
      ...
      <main className="booking-content">
        ...
        {step === 2 && <section className="booking-step-content" aria-labelledby="date-title"><h1 id="date-title">Cuando queres venir?</h1><p>Eligi empleado, fecha y hora</p><h2 className="booking-subtitle">Con quien?</h2><div className="booking-options barber-options">{barbers.map((barber) => <button className={booking.barber === barber.name ? 'is-selected' : ''} key={barber.name} type="button" onClick={() => updateBooking('barber', barber.name)}><b>{barber.initials}</b><span><strong>{barber.name}</strong><small>{barber.detail}</small></span></button>)}</div><h2 className="booking-subtitle">Septiembre 2026</h2><div className="calendar" role="group" aria-label="Calendario de septiembre de 2026"><div className="calendar-weekdays">{weekDays.map((day) => <span key={day}>{day}</span>)}</div><div className="calendar-grid">{Array.from({ length: 2 }, (_, index) => <span className="calendar-empty" key={`empty-${index}`} />)}{calendarDays.map((day) => <button className={`${booking.date === getDateLabel(day) ? 'is-selected' : ''} ${day === 3 ? 'is-today' : ''}`} disabled={day < 3} key={day} type="button" onClick={() => { updateBooking('date', getDateLabel(day)); updateBooking('time', '') }}>{day}</button>)}</div></div>{booking.date && <><h2 className="booking-subtitle">Horarios disponibles</h2><div className="booking-options time-options">{times.map((time) => <button className={booking.time === time ? 'is-selected' : ''} key={time} type="button" onClick={() => updateBooking('time', time)}>{time}</button>)}</div></>}<div className="form-actions"><button type="button" onClick={() => setStep(1)}>Atras</button><button className="form-submit" type="button" disabled={!canContinue} onClick={() => setStep(3)}>Continuar</button></div></section>}
